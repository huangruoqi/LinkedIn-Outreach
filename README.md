...
- Pre-allows every LinkedIn MCP tool in Claude Code's settings (`mcp__linkedin`) so the first call does not block on a permission prompt. Writes to **`~/.claude/settings.json`** in default mode, or **`<repo>/.claude/settings.local.json`** with **`--local`**.
